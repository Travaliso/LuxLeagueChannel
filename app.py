...
# ------------------------------------------------------------------
# 4. DATA PIPELINE (Pre-load data for the view)
# ------------------------------------------------------------------
...
if selected_page == P_LEDGER:
    ...
    for i, m in enumerate(matchup_data):
        with c1 if i % 2 == 0 else c2:
            ...
            with st.expander(f"📉 View Lineups"):
                # Convert roster data to DF for display
    ...
    if "rank_comm" not in st.session_state:
        with utils.luxury_spinner("Analyzing hierarchy..."): st.session_state["rank_comm"] = utils.get_rankings_commentary(OPENAI_KEY, df_eff.iloc[0]['Team'], df_eff.iloc[-1]['Team'])
    ...
    try:
        ...
        if "next_week_comm" not in st.session_state:
            with utils.luxury_spinner("Checking Vegas lines..."): st.session_state["next_week_commentary"] = utils.get_next_week_preview(OPENAI_KEY, games)
