...
st.markdown("""
    ...
    .award-blurb { color: #a0aaba; font-size: 0.85rem; margin-top: 15px; font-style: italic; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 10px; width: 100%; }
...
def run_monte_carlo_simulation(_league, simulations=1000):
    ...
    for team in _league.teams:
        # FIX: SCALE ODDS TO 0-1 FOR PROGRESS COLUMN
...
# --- F. NEXT GEN STATS ENGINE (UPDATED V3 with Fallback) ---
...
def analyze_nextgen_metrics_v3(roster, year):
    ...
    for player in roster:
        ...
        if pos in ['WR', 'TE'] and not df_rec.empty:
            # FIX: Correct unpacking of 2 values from thefuzz
...
def get_next_week_preview(games_list):
    ...
    return ai_response(f"Act as a Vegas Sports Bookie. Provide a detailed preview of next week's matchups: {matchups_str}. Pick 'Lock of the Week' and 'Upset Alert'.", 800)

def get_season_retrospective(mvp, best_mgr):
    return ai_response(f"Write a 'State of the Union' address for the league. MVP: {mvp}. Best Manager: {best_mgr}. Style: Presidential.", 1000)

def get_ai_trade_proposal(team_a, team_b, roster_a, roster_b):
    return ai_response(f"Act as Trade Broker. Propose a fair trade between Team A ({team_a}): {roster_a} and Team B ({team_b}): {roster_b}. Explain why.", 600)
...
# ------------------------------------------------------------------
# 7. RENDER HERO ROW (DATA DEPENDENT)
# ------------------------------------------------------------------
...
if selected_page == P_LEDGER:
    ...
    if "draft_roi" not in st.session_state:
        ...
        if not df_roi.empty:
            ...
            with c1:
                ...
                st.dataframe(penny_stocks[["Player", "Team", "Round", "Points"]], use_container_width=True, hide_index=True)
            with c2:
                st.subheader("💸 Bad Debt")
                bad_debt = df_roi[df_roi["Round"] <= 3].sort_values(by="Points", ascending=True).head(10)
                st.dataframe(bad_debt[["Player", "Team", "Round", "Points"]], use_container_width=True, hide_index=True)
        else:
    ...
    with st.form("multiverse_form"):
        ...
        for i, g in enumerate(box):
            ...
            with col:
                st.markdown(f"**{g.home_team.team_name}** vs **{g.away_team.team_name}**")
                choice = st.radio("Result:", ["Simulate", f"{g.home_team.team_name} Win", f"{g.away_team.team_name} Win"], key=f"g{i}", label_visibility="collapsed", horizontal=True)
                if "Home" in str(choice): forced.append(g.home_team.team_name)
                elif "Away" in str(choice): forced.append(g.away_team.team_name)
        st.markdown("---")
        run_scenario = st.form_submit_button("🔮 Enter The Multiverse")

    if run_scenario:
        with luxury_spinner("Simulating Alternate Timeline..."):
            df_scenario = run_multiverse_simulation(league, forced_winners_list=forced)
    ...
    try:
        ...
        if "next_week_commentary" not in st.session_state:
            with luxury_spinner("Checking Vegas lines..."): st.session_state["next_week_commentary"] = get_next_week_preview(games)
    ...
    if "awards" not in st.session_state:
        ...
        st.divider()
        def gen_nar(type, team, value, extra=""):
            if type == "Oracle": return f"The ultimate strategist. {team} achieved a staggering **{value:.1f}% efficiency rating**."
            if type == "Sniper": return f"Leagues are won on the wire. {team} extracted **{value:.1f} points** from free agents."
            if type == "Purple": return f"A season defined by grit. {team} managed **{value} injury designations**."
            if type == "Hoarder": return f"An embarrassment of riches. {team} left **{value:.1f} points** on the bench."
            if type == "Toilet": return f"The offense couldn't launch. Scoring only **{value:.1f} total points**."
            if type == "Blowout": return f"A historic beatdown. {team} lost by **{value:.1f} points**."
            return ""
