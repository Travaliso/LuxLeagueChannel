...
if 'box_scores' not in st.session_state or st.session_state.get('week') != selected_week:
    with ui.luxury_spinner(f"Accessing Week {selected_week} Data..."):
        try:
            ...
            # Clear stale AI reports whenever the week changes so it doesn't get stuck on 0.0 scores
            for k in ["recap", "rank_comm", "next_week_comm"]:
            ...
            try:
                ...
                for m in matchup_data:
                    clean_matchup_data.append({
                        "Home": m["Home"], "Home Score": m["Home Score"], "Home Roster": m["Home Roster"],
                        "Away": m["Away"], "Away Score": m["Away Score"], "Away Roster": m["Away Roster"]
                    })
