...
if selected_page == P_LEDGER:
    ...
    if "draft_roi" not in st.session_state:
        ...
        st.markdown(f"""<div class="luxury-card" style="border-left: 4px solid #92FE9D; display: flex; align-items: center;"><div style="flex: 1; text-align: center;"><img src="{prescient['Logo']}" style="width: 90px; border-radius: 50%; border: 3px solid #92FE9D;"></div><div style="flex: 3; padding-left: 20px;"><h3 style="color: #92FE9D; margin: 0;">The Prescient One</h3><div style="font-size: 1.8rem; font-weight: 900; color: white;">{prescient['Team']}</div><div style="color: #a0aaba;">Generated <b>{prescient['Points']:.0f} points</b> from waivers while securing <b>{prescient['Wins']} Wins</b>.</div></div></div>""", unsafe_allow_html=True)
        if not df_roi.empty:
    ...
    with st.form("multi"):
        ...
        for i, g in enumerate(box):
            with c1 if i % 2 == 0 else c2:
                c = st.radio(f"{g.home_team.team_name} vs {g.away_team.team_name}", ["Sim", f"{g.home_team.team_name}", f"{g.away_team.team_name}"], key=f"g{i}", horizontal=True)
                if c != "Sim": forced.append(c)
    ...
    if "multi_res" in st.session_state:
        st.dataframe(st.session_state["multi_res"], use_container_width=True, hide_index=True, column_config={"New Odds": st.column_config.ProgressColumn("Odds", min_value=0, max_value=100), "Impact": st.column_config.NumberColumn(format="%+.1f%%")})

elif selected_page == P_NEXT:
    box = league.box_scores(week=league.current_week)
    if "next_comm" not in st.session_state:
        with utils.luxury_spinner("Checking Vegas..."): 
            games = [{"home": g.home_team.team_name, "away": g.away_team.team_name, "spread": str(abs(g.home_projected-g.away_projected))} for g in box]
            st.session_state["next_comm"] = utils.get_next_week_preview(OPENAI_KEY, games)
    st.markdown(f'<div class="luxury-card studio-box"><h3>🎙️ Vegas Insider</h3>{st.session_state["next_comm"]}</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    for i, g in enumerate(box):
         with c1 if i % 2 == 0 else c2:
    ...
    if "awards" not in st.session_state:
        if st.button("🏅 Unveil Awards"):
            with utils.luxury_spinner("Engraving..."):
                ...
                st.session_state["season_comm"] = utils.get_season_retrospective(OPENAI_KEY, st.session_state["awards"]['MVP']['Name'], st.session_state["awards"]['Best Manager']['Team'])
                st.rerun()
    else:
