...
for game in box_scores:
    ...
    efficiency_data.append({"Team": home.team_name, "Total Potential": sum(p['Score'] for p in h_r)+sum(p['Score'] for p in h_br), "Starters": sum(p['Score'] for p in h_r), "Bench": sum(p['Score'] for p in h_br)})
    efficiency_data.append({"Team": away.team_name, "Total Potential": sum(p['Score'] for p in a_r)+sum(p['Score'] for p in a_br), "Starters": sum(p['Score'] for p in a_r), "Bench": sum(p['Score'] for p in a_br)})
...
if selected_page == P_LEDGER:
    ...
    if st.session_state.get("trigger_lab"):
        # FIX: PASS 'league' AND 'current_week'
        roster_obj = next(t for t in league.teams if t.team_name == target_team).roster
        st.session_state["ngs_data"] = logic.analyze_nextgen_metrics_v3(roster_obj, league, current_week)
            ...
            for i, row in st.session_state["ngs_data"].iterrows():
                with cols[i % 2]:
                    ...
                    if st.button(f"🧠 Assistant GM", key=f"lab_{row['ID']}"):
                         ...
                         assessment = intel.get_lab_assessment(OPENAI_KEY, row['Player'], row['Team'], row['Position'], row['Opponent'], matchup_rank, f"{row['Metric']}: {row['Value']} ({row['Alpha Stat']})", vegas_line, row['ESPN Proj'])
                         st.info(assessment)
    ...
    with st.form("multi_form"):
        for i, g in enumerate(box):
            c = st.radio(f"{g.home_team.team_name} vs {g.away_team.team_name}", ["Sim", f"{g.home_team.team_name} Win", f"{g.away_team.team_name} Win"], key=f"g{i}", horizontal=True)
