...
def analyze_nextgen_metrics_v3(roster, league, current_week):
    # UPDATED TO ACCEPT LEAGUE & WEEK (Fix for missing opponent context)
    year = league.year
    df_rec, df_rush, df_pass, df_seas = load_nextgen_data_v3(year)
    if df_rec is None or df_rec.empty: return pd.DataFrame()
    
    # Build Opponent Map from Box Scores (Robust Method)
    team_opp_map = {}
    box = league.box_scores(week=current_week)
    for game in box:
        # Map home team -> away team abbrev, and vice versa
        h_tm = game.home_team.team_name
        a_tm = game.away_team.team_name
        h_abbr = clean_team_abbr(game.home_team.team_abbrev)
        a_abbr = clean_team_abbr(game.away_team.team_abbrev)
        
        team_opp_map[h_tm] = a_abbr
        team_opp_map[a_tm] = h_abbr
        
        # Also map proTeam abbreviations if possible (ESPN sometimes uses different ones)
        # This is a best-effort mapping. Relying on team name is safer.
    ...
    for player in roster:
        ...
        p_team_name = "Free Agent" # Default
        
        # Find which team this player is on (roster object is a list of players, need to find team)
        # Actually, 'roster' passed here is just a list of players from ONE team in the app.py call.
        # So we can't easily find the team name from the player object itself if it's just a Player object.
        # Wait, Player object has 'proTeam'. But we need the Fantasy Team to find the opponent from the schedule?
        # No, we need the NFL Team (proTeam) to find the NFL Opponent.
        
        p_pro_team = clean_team_abbr(getattr(player, 'proTeam', 'UNK'))
        
        # Find NFL Opponent using Schedule (We need a pro-team schedule map)
        # This is hard without a full NFL schedule loaded.
        # FALLBACK: Use the 'proOpponent' attribute if available, or look at box scores if we can match proTeam.
        
        opp = clean_team_abbr(getattr(player, 'proOpponent', 'UNK'))
        
        # If 'proOpponent' is missing, try to find it in the box scores (if we scraped them in get_vegas_props)
        # But here we are in a different function.
