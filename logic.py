...
def clean_team_abbr(abbr):
    # Maps ESPN/weird codes to Standard NFL codes
    mapping = {
...
def get_vegas_props(api_key, _league, week):
    # This function is used by the Prop Desk, but we are fixing the Lab here.
    # Included to ensure file completeness.
    url = 'https://api.the-odds-api.com/v4/sports/americanfootball_nfl/events/upcoming/odds'
    params = {'apiKey': api_key, 'regions': 'us', 'markets': 'player_pass_yds,player_rush_yds,player_reception_yds,player_anytime_td', 'oddsFormat': 'american'}
    try:
        ...
        for team in _league.teams:
            for p in team.roster:
                espn_map[normalize_name(p.name)] = {"name": p.name, "id": p.playerId, "pos": p.position, "team": team.team_name, "espn_proj": getattr(p, 'projected_points', 0)}
             ...
             for bm in event['bookmakers']:
                 for mkt in bm['markets']:
                     for out in mkt['outcomes']:
                         ...
                         if norm in espn_map:
                             # Simplified for Logic file context
                             props.append({"Player": espn_map[norm]['name'], "Proj Pts": 0, "Edge": 0, "Verdict": "Hold"})
...
# --- UPDATED LAB LOGIC WITH SCHEDULE ---
def analyze_nextgen_metrics_v3(roster, year, current_week):
    ...
    for player in roster:
        ...
        # DETERMINE OPPONENT FROM SCHEDULE MAP
