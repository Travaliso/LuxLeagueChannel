...
def inject_luxury_css():
    ...
    h1 { color: #ffffff !important; font-family: 'Helvetica Neue', sans-serif; font-weight: 800 !important; text-shadow: 0 2px 4px rgba(0,0,0,0.5); }
    h2, h3, h4 { color: #ffffff !important; font-weight: 700 !important; }
    ...
    @media (max-width: 768px) {
        ...
        .luxury-card { padding: 15px; margin-bottom: 10px; }
        .award-card { min-height: auto !important; padding: 20px !important; }
        
        /* Force Horizontal Scroll for Podium/Elite (Optional) */
        /* Note: Streamlit columns force stack on mobile. This CSS attempts to keep things cleaner. */
    }
    ...
    div[role="radiogroup"] label { padding: 12px 15px !important; border-radius: 10px; margin-bottom: 5px; transition: all 0.3s; border: 1px solid transparent; }
    div[role="radiogroup"] label:hover { background-color: rgba(255, 255, 255, 0.05); }
    div[role="radiogroup"] label[data-checked="true"] { background: linear-gradient(90deg, rgba(0, 201, 255, 0.15), transparent); border-left: 4px solid #00C9FF; font-weight: bold; color: white; }
    div[role="radiogroup"] label > div:first-child { display: none; }
    
    /* --- TABLE STYLING --- */
    div[data-testid="stDataFrame"] { background-color: rgba(17, 25, 40, 0.5); border-radius: 15px; padding: 10px; border: 1px solid rgba(255,255,255,0.05); }
    ...
    /* Mobile Loader Text Fix */
    @media (max-width: 768px) {
        .luxury-loader-text { font-size: 2rem !important; letter-spacing: 4px !important; }
    }
...
def get_ai_scouting_report(key, free_agents_str):
    return ai_response(key, f"You are an elite NFL Talent Scout. Analyze free agents: {free_agents_str}. Identify 3 'Must Adds'. Style: Scouting Notebook.", 500)
...
def get_rankings_commentary(key, top, bottom):
    return ai_response(key, f"Write a 5-8 sentence commentary on Power Rankings. Praise {top} and mock {bottom}. Style: Stephen A. Smith.", 600)
...
def get_ai_trade_proposal(key, t1, t2, r1, r2):
    return ai_response(key, f"Act as Trade Broker. Propose a fair trade between Team A ({t1}): {r1} and Team B ({t2}): {r2}. Explain why.", 600)
